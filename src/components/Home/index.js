...
function CourseListItem(props) {
    ...
    //TODO: REMOVE AFTER LEANGAP IS DONE
    // if (code.includes('KOIN')) {
    //     return null;
    // }
    return ( 
        <div className={props.active+"courselistitem"} onClick={() => {props.changeActiveCourse(props.name); setTimeout(() => window.dispatchEvent(new Event('resize')), 250)}} >
            {code}
        </div>
    );
}
            ...
            <div className={attribute}>
                <div className="showflex" >
                    ...
                    <div className="showicon">
                        <label className={this.state.progressMark ? "checkedswitchcheck" : "switchcheck"} onClick={() => this.markProgress(!(this.state.progressMark))}>
                            {/* {(this.state.progressMark) ? ('✔') : ('')} */}
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div className="moduletitle" onClick={() => setActive(active==="active" ? "" : "active")}>
            <div className={("normal")}>
                <div className="bisplitbetween">
                    <div>{active ? "▴" : "▾"} &nbsp;&nbsp; {props.name}</div>
                    <div className="bisplitbetweenright"><div className="expltext" onMouseOver={() => setShowExpl(true)} onMouseOut={() => setShowExpl(false)}>{(showExpl) ? ('Mark as Complete') : ('( ? )')}</div></div>
                </div>
            </div>
        </div>
    );
...
class MainPanel extends React.Component {
    ...
    addTeacher(uid) {
		...
		this.props.firebase.user(uid).on('value', snapshot => {
            if (snapshot.val() == null) alert ("User does not exist!")
                ...
                if (snapshot2.val().time === 1000000) alert('This user is not a Modulus Pro user yet!')
                else {
    ...
    render() { //active: "" means the module is minimized, "active" means its expanded
        ...
        if (this.props.activeCourse==="none") {
            welcomeMsg = (
                <>
                    No course selected.
                    <br /><br />
                    {(this.props.isMobile==='large') ? (<button className="tourbutton" onClick={this.showTour}>
                        Take a Tour &nbsp; &rsaquo;
                    </button>) : (null)}
                    {/* <br /><br /><br />
            ...
            welcomeMsg = this.codeToName(this.props.activeCourse); //CodeToName
                        ...
                        (listOfStudentKicks[listOfStudents.indexOf(student)]==="false") ? (
                            <div className={shade[listOfStudents.indexOf(student)]}>
                                <div className="studentcell" onMouseOver={() => this.setState({displayexpl: true})} onMouseOut={() => this.setState({displayexpl: false})} onClick={() => ModalManager.open(<ProgressModal modules={this.props.modules} firebase={this.props.firebase} activeCourse={this.props.activeCourse} studentName={this.getStudentNameFromEmail(student)} getStudentProgress={this.getStudentProgress} studentEmail={student} onRequestClose={() => true}/>)}>
                                    <p className="namedstudentsmallparagraph">
        ...
        if (this.props.isTimeUp()===0 && teacherMode) { //if paywalled as a teacher
            ...
            return (
                <div className={atmainpanel}>
                    ...
                    <h1 className={atcourseheader}>
                        <div className="bisplit">
                            <div className="bisplitpart">
                                {welcomeMsg}
                            </div>
                            {/* {(this.props.isMobile!=='large') ? (null) : (<div className="rightbisplitpart">
                                {(subjectTag)}
                            </div>)} */}
                        </div>
                    </h1>
        ...
        if (this.state.stage===-1) { //student only wall
            return (
                <div className="studentonlywall">
                    <h1>Creating a course is a Pro feature.</h1>
                    <h2>To proceed, you'll enter a Free Trial of Modulus Pro.</h2>
                    <h3>This unlocks all of our Pro features for 30 days.</h3>
        ...
        <div className="mainpaneldialog">
            <div className="addcourseview">
                <div className="addcoursetitle">
                    <b>Browse Public Courses</b>
                </div>
                Learn anything, with our open library of free courses.
                <div className="searchstrip">
                    <div className="searchflexsplit">
                        ...
                        <SearchSelect passState={passState}/>
                    </div>
                </div>
                {
                            ...
                            <div className="flexsplit">
                                <div className="leftsplit">
                                    ...
                                    <center>
                                        ...
                                        <br />
                                        <button onClick={() => props.addCourse(course.joinCode)}>
                                            Enroll
                                        </button>
                                    </center>
                                    <br />
...
class Container extends React.Component { 
    ...
    addCourseMode() { //switches main panel view to addCourseMode
        var actionStr = "" + this.state.username + " clicked on Enroll";
        ReactGA.event({    
            category: "Clicked Enroll",
            action: actionStr,
        });
    ...
    createCourseMode() { //switches main panel view to createCourseMode
        var actionStr = "" + this.state.username + " clicked on Create";
        ReactGA.event({    
            category: "Clicked Create",
            action: actionStr,
        });
    ...
    browseCourseMode() { //switches main panel view to browseCourseMode
        var actionStr = "" + this.state.username + " clicked on Browse";
        ReactGA.event({    
            category: "Clicked Browse",
            action: actionStr,
        });
    ...
    addCourse = (courseCode) => { //actually adds the course
        ...
        if (newCourses.includes(courseCode)) {
            ...
            }, () => {this.props.updateCourses(newCourses.slice()); this.changeActiveCourse(courseCode); });
            const usr = secureStorage.getItem('authUser');
    ...
    createCourse = (nameOfCourse, courseVisibility, courseDescription) => { //actually adds the course
        ...
        const tempName = username + classCode;
        const tempStudents = {
    ...
    removeCourse = (courseCode) => {
        var actionStr = "" + this.state.username + " unenrolled from " + courseCode;
        ReactGA.event({
            category: "Course Unenrolled",
            action: actionStr,
        });
    ...
    render() {
        ...
        if (this.state.mainPanelMode===0) {
            ...
            mainpanel = <PublicCourse addCourse={this.addCourse} firebase={this.props.firebase} />
        }
        ...
        var leftelement = (
            <div className={leftelementattribute}>
                <ResizableBox width={500} height={0} axis={'x'} resizeHandles={['ne']}
                ...
                <div className="sidebar">
                    <div className="courselist">
                        ...
                        <center>
                            {(this.state.arrCourses.length<=1) ? (
                                <p className="smallemptytext">You're not enrolled in any courses yet.</p>
                            ) : 
        ...
        if (this.props.isMobile==='medium') {
            button = (this.state.hidden) ? (
                <button className={(!(this.props.isMobile==="medium")) ? ("smallsidebartoggle") : ("mobile-smallsidebartoggle")} onClick={this.toggleHidden}>
                    <img className={("flipimage")} src={menuicon} width="12px" alt="Sidebar Toggle"/> 
                </button>
            ) : (
            ...
            button = (this.state.hidden) ? (
                <button className={(!(this.props.isMobile==="medium")) ? ("smallsidebartoggle") : ("mobile-smallsidebartoggle")} onClick={this.toggleHidden}>
                    <img className={("flipimage")} src={backarrow} width="12px" alt="Sidebar Toggle"/> 
                </button>
            ) : (
                <button className={(!(this.props.isMobile==="medium")) ? ("sidebartoggle") : ("mobile-sidebartoggle")} onClick={this.toggleHidden}>
